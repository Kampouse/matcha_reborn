...
export function createServer(opts: ServerOptions) {
  ...
  void server.register(fastifyTRPCPlugin, {
    ...
    useWSS: true,
    trpcOptions: {
      router: appRouter, createContext, Headers: {
  ...
  server.get('/register', async (req, res) => {
    const validated_input = z.object({ email: z.string(), username: z.string(), password: z.string(), re_password: z.string() }).parse(req.body)
    ...
    if (userExist) {
      ...
      res.send({ email: validated_input.email, username: validated_input.username });
