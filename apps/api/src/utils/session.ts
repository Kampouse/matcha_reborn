...
export const ValidateUser = async (input: string, mail: string) => {
  try {
    ...
    const content = await Clientdb.execute("SELECT username,email FROM users WHERE email = ? AND password_hash  = ?", [mail, hashed])
...
export const isExistinUser = async (inputEmail: string) => {
  const datUser = await Clientdb.execute("SELECT username,email FROM users WHERE email = ? ", [inputEmail])
  if (datUser.rows.length === 0) {
    return false
  }
  return true
}
export const createUser = async (input: { email: string, username: string, password: string }) => {
  const hashed = await UsePasswordFromHash(input.password)
  const content = await Clientdb.execute("INSERT INTO users (email,username,password_hash) VALUES (?,?,?)", [input.email, input.username, hashed])
